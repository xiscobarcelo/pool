...
// ============================================
// ✅ NUEVA FUNCIÓN: DETECTAR POSICIÓN (MEJORADA)
// Prioriza Campeón > Subcampeón > Semifinales > Números
// ============================================
...
function detectTournamentPosition(result, position) {
    ...
    if (position) {
        ...
        detectedPosition = `${positionNumber}º`;
    ...
    // ORDEN IMPORTA - De más específico a menos
    
    // 1. Subcampeón / Finalista / 2º
    ...
    // 2. Semifinales (puede ser 3º o 4º)
    ...
    // 6. Intentar extraer cualquier número
    else {
        const match = result.match(/(\d+)/);
        if (match) {
            positionNumber = parseInt(match[1]);
...
// ============================================
// ✅ CALCULAR ESTADÍSTICAS DE CIRCUITOS (FIXED)
// Ahora usa detectTournamentPosition para mejor detección
// ============================================
...
function calculateCircuitStats() {
    ...
    selectedCircuits.forEach(circuitId => {
        ...
        // ✅ FIXED: Calcular posición promedio y mejor posición con detección mejorada
