        ...
        // ============================================================
// CÁLCULO UNIFICADO DE ESTADÍSTICAS
// Suma: Partidos Importados + Partidos Manuales + Stats Manuales
// ============================================================

// ✅ ESTA es la función que debes usar en estadisticas.js
function calculateUnifiedStats(matches, modalityStats) {
console.log(‘📊 Calculando estadísticas unificadas…’);

```
// 1️⃣ Calcular desde los PARTIDOS registrados (importados + añadidos manualmente)
const matchStats = calculateStatsFromMatches(matches);

console.log('📦 Stats de partidos:', matchStats);
console.log('📦 Stats manuales:', modalityStats);

// 2️⃣ Si NO hay stats manuales, devolver solo las de partidos
if (!modalityStats) {
    console.log('✅ Solo hay partidos, sin stats manuales');
    return matchStats;
}

// 3️⃣ SUMAR partidos + stats manuales
const unifiedStats = {
    bola8: {
...
// Para los cards destacados de estadísticas y historial
// ============================================================

function calculateTotalStats(unifiedStats) {
let totalMatches = 0;
...
['bola8', 'bola9', 'bola10'].forEach(mod => {
    totalMatches += unifiedStats[mod].matchesPlayed || 0;
    totalMatchesWon += unifiedStats[mod].matchesWon || 0;
    totalGames += unifiedStats[mod].gamesPlayed || 0;
    totalGamesWon += unifiedStats[mod].gamesWon || 0;
});
...
/*
// En estadisticas.js o historial.js:

// 1. Obtener datos
const data = CloudSync.getData();

// 2. Calcular stats unificadas
const unifiedStats = calculateUnifiedStats(data.matches, data.modalityStats);
...
document.getElementById(‘totalMatchesWon’).textContent = totals.totalMatchesWon;
document.getElementById(‘winRate’).textContent = totals.winRate + ‘%’;
document.getElementById(‘totalGames’).textContent = totals.totalGames;
document.getElementById(‘totalGamesWon’).textContent = totals.totalGamesWon;

// 5. Usar unifiedStats para gráficos
...
// ========================================
        ...
        function loadModalityData(modalityStats, matches) {
            ...
            syncInfo.style.cssText = 'background: linear-gradient(135deg,rgba(245, 247, 250, 0) 0%, rgba(232, 236, 241, 1) 100%); border: 1px solid rgba(52, 199, 89, 0.3); border-radius: 12px; padding: 16px; margin-bottom: 24px; text-align: center;';
            syncInfo.innerHTML = `
...
function updateComparison(matches) {
    ...
    html = `
        <div style="background: linear-gradient(135deg,rgba(245, 247, 250, 0) 0%, rgba(232, 236, 241, 1) 100%);
